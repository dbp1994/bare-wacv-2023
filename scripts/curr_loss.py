...
parser.add_argument('-burn', '--burn_in', default=10, type=int, help="burn-in period (#epochs)")
...
def risk_min_train(dataset_train, train_loader, model):
    ...
    for batch_id, (x, y, idx) in tqdm(enumerate(train_loader)):
        ...
        if epoch > burn_in:
            ...
            loss_tmp, ind_sort = torch.sort(loss_batch, 0)
            tmp = 0.
            ind_tmp = []
            C = (1 - noise_rate)*y.shape[0]
            ...
            idx_tmp = torch.index_select(ind_sort,0,
                                         torch.Tensor(ind_tmp).type(torch.LongTensor).to(device))
            loss_batch = torch.index_select(loss_batch, 0, idx_tmp)

            idx_sel = torch.zeros(x.shape[0])
            idx_sel[idx_tmp] = 1.
            idx_sel_tr_agg[list(map(int,idx.tolist()))] = np.asarray(idx_sel.tolist())

        else:
            idx_sel = torch.ones(x.shape[0])
            idx_sel_tr_agg[list(map(int, idx.tolist()))] = np.asarray(idx_sel.tolist())
...
for run in range(num_runs):
    ...
    chkpt_path = f"./checkpoint/{mode}/{dataset}/{noise_type}/\
                  0{str(int(noise_rate*10))}/run_{str(run)}/"

    res_path = f"./results_pkl/{mode}/{dataset}/{noise_type}/\
                0{str(int(noise_rate*10))}/run_{str(run)}/"

    plt_path = f"./plots/{mode}/{dataset}/{noise_type}/\
                0{str(int(noise_rate*10))}/run_{str(run)}/"

    log_dirs_path = f"./runs/{mode}/{dataset}/{noise_type}/\
                     0{str(int(noise_rate*10))}/run_{str(run)}/"
    ...
    print("file name: %s-aug-%s-%s-%s-%s-nr-0%s-\
           run-%s.pt" % (mode, data_aug, dataset,
    ...
    for epoch in range(num_epoch):
        ...
        loss_train, acc_train, idx_sel_tr_agg = risk_min_train(dataset_train,
                                                               train_loader,
                                                               model)
        ...
        print("Epoch: {}, lr: {}, loss_train: {}, loss_val: {},\
              loss_test: {:.3f}, acc_train: {}, acc_val: {}, \
